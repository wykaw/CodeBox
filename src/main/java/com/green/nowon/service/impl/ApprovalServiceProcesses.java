...
@Service
public class ApprovalServiceProcesses implements ApprovalService {
	...
	public void detail(long ano, Model model) {
		var apEn = repo.findById(ano);
		if (!apEn.isPresent()) return;
		model.addAttribute("list", repo.findById(ano).map(ApprovalListDTO::new).orElseThrow());
		List<DepartmentMemberEntity> abc = dRepo.findByMemberMno(repo.findById(ano).orElseThrow().getMno().getMno());
